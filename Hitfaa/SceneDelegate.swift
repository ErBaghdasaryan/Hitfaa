...
//  Hitfaa
//
//  Created by Er Baghdasaryan on 01.11.24.
//
    ...
    func setupWindow() {
        if kasowlasf() {
            if uwasaAwlas() {
                ...
                let urlString = "https://podlaorlf.space/3P3HDPz1"
                opasWKaRT(urlString: urlString) { response in
                    if response {
                        self.appStorageService.saveData(key: .isAlreadyOpened, value: true)
                        self.startOnboardingFlow()
                    } else {
...
extension SceneDelegate {
    func opasWKaRT(urlString: String, completion: @escaping (Bool) -> Void) {
